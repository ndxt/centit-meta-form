    ...
    public JSONArray searchFormModeAsJson(String keyWords, String applicationId, String formType, PageDesc pageDesc) {
        ...
        if(this.getDBtype() == DBType.MySql) {
            if ("mobile".equals(formType)) {
                sql = sql + " and (match (a.MOBILE_FORM_TEMPLATE) against( ? IN BOOLEAN MODE) or match (STRUCTURE_FUNCTION) against( ? IN BOOLEAN MODE))";
            } else {
                sql = sql + " and (match (a.form_template) against( ? IN BOOLEAN MODE) or match (STRUCTURE_FUNCTION) against( ? IN BOOLEAN MODE))";
            }
            sql = sql + " order by a.LAST_MODIFY_DATE desc";
            JSONArray listTables = DatabaseOptUtils.listObjectsBySqlAsJson(
                    this, sql, new Object[]{applicationId, pretreatmentQueryWord(keyWords),pretreatmentQueryWord(keyWords)}, pageDesc);
            return listTables;
        }
        ...
        sql = sql + " order by a.LAST_MODIFY_DATE desc";
        JSONArray listTables = DatabaseOptUtils.listObjectsBySqlAsJson(
                this, sql, new Object[]{applicationId, QueryUtils.getMatchString(keyWords),QueryUtils.getMatchString(keyWords)}, pageDesc);
        return listTables;
