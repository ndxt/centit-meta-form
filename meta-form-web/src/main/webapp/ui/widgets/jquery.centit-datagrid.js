    ...
    var parseConfirmButton = function (opts, mainCtrl, subCtrl) {
      ...
      var btn = {
        ...
        handler: function () {
          ...
          if(opts.checkbox){
            title=title+'<div class="message-checkbox">'+opts.checkbox+'<input type="checkbox"/>'+'</div>';
            $.messager.confirm(opts.text, title, function (r) {
              var deleteDataBase = $(".message-checkbox>input").is(":checked");
              if (r&&deleteDataBase) {
                subCtrl.submit.call(subCtrl, table, $.extend({},result,{'deleteDataBase':deleteDataBase}));
              }
              else {
                if (r) {
    ...
    var _parseButtonOptions = function (target) {
      ...
      return $.extend({}, $.fn.linkbutton.parseOptions(target),
        ...
        $.parser.parseOptions(target, ['for', 'columnOperatorText',
          'rel', 'trigger', 'target', 'title', 'warn', 'href', 'btnValue', 'buttons', 'size','checkbox',
